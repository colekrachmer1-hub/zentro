...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    serpapi_key: data?.serpapi_key ? '••••••••' + data.serpapi_key.slice(-4) : null,
  })
}
...
export async function POST(req: NextRequest) {
  ...
  const { openai_key, anthropic_key, serpapi_key } = await req.json()
  ...
  if (serpapi_key && !serpapi_key.includes('•')) updates.serpapi_key = serpapi_key.trim()
